...
public class WallpaperChangerOld {
    ...
    public static void main(String[] args) throws InterruptedException, Exception, IOException {
            //另一個WallpaperChangerOld類別是使用另一種寫法來設置桌布，效果看似一樣
        String mp4Path="C:\\oozora_subaru.mp4";     //影片位址
        File imgPath=new File("convert");   //轉換到的資料夾名稱
        imgPath.delete();
        imgPath.mkdirs();
        String imagePath=imgPath.getAbsolutePath();
        //convertMovietoJPG(mp4Path, imagePath,"jpg",0);    //將影片轉成圖片，第一次用請開啟

        System.out.println("Conversion complete. Please find the images at "+imagePath);
        playWallpaper(imagePath,5000);  //設置桌布
    }
}